 ...
 */
public abstract class BaseDeploymentGithubParser extends BaseGithubParser {
  ...
  protected String getMessage(Map<String, String> parameters, JsonNode node)
    ...
    try {
      ...
      throw new GithubParserException(
          "Something went wrong while building the message for Github deployment event.", e);
    }
  }
   ...
   * @throws URISyntaxException Reports failure to deal with the urls inside the deployment json
   * node
   */
   ...
   * @param builder EntityBuilder to insert the information about the creator, repository and
   * sender.
